...
class GeminiProvider(LLMProvider):
    """
    Google Gemini provider with model-first rotation.

    Rotation order: models first, then keys.
    model[0]+key[0] → model[1]+key[0] → model[0]+key[1] → model[1]+key[1] → exhausted
    """
    ...
    def _advance(self) -> bool:
        """Advance to next model, then key. Returns False if exhausted."""
        self._current_model_idx += 1
        if self._current_model_idx < len(self._models):
            return True
        # Models exhausted for current key, advance key
        self._current_model_idx = 0
